...
def render(user=None) -> None:
    """Render the quotation workspace for an authenticated user.

    ``user`` is accepted so the shell can pass the signed-in principal; the
    trusted identity is still resolved from the session on every service call.
    """
...
def main() -> None:
    ...
    st.title(":material/request_quote: Create quotation")
    st.caption(
        "Internal workspace — cost, margin and approval thresholds shown here "
        "are never part of the customer document."
    )
    ...
    with conversation_column:
        st.subheader("Requirements conversation")
...
def _render_approval_submission(state: QuotationWorkflowState) -> None:
    ...
    if user is None:
        st.warning(
            "Sign in to submit this quotation for review.",
            icon=":material/lock:",
        )
        return
